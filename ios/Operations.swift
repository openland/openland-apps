...
class Operations {
    ...
    let ChatJoin = OperationDefinition(
        "ChatJoin",
        .query, 
        "query ChatJoin($id:ID!){room(id:$id){__typename ... on SharedRoom{description id isChannel membersCount onlineMembersCount photo previewMembers{__typename id name photo}title}}}",
        ChatJoinSelector
    )
    ...
    let GetUser = OperationDefinition(
        "GetUser",
        .query, 
        "query GetUser($shortname:String!){user:alphaResolveShortName(shortname:$shortname){__typename ... on User{...UserNano}}}fragment UserNano on User{__typename firstName id lastName name online photo}",
        GetUserSelector
    )
    ...
    let UserAvailableRooms = OperationDefinition(
        "UserAvailableRooms",
        .query, 
        "query UserAvailableRooms($after:String,$first:Int!,$query:String){alphaUserAvailableRooms(after:$after,first:$first,query:$query){__typename edges{__typename cursor node{__typename ... on SharedRoom{id kind membersCount membership organization{__typename id name photo}photo title}}}pageInfo{__typename hasNextPage}}}",
