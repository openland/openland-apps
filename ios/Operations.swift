...
class Operations {
    ...
    let FeedChannel = OperationDefinition(
        "FeedChannel",
        .query, 
        "query FeedChannel($id:ID!){channel:alphaFeedChannel(id:$id){__typename ...FeedChannelFull}}fragment FeedChannelFull on FeedChannel{__typename about id isGlobal myRole photo shortname socialImage subscribed subscribersCount title}",
        FeedChannelSelector
    )
    ...
    let FeedMyChannels = OperationDefinition(
        "FeedMyChannels",
        .query, 
        "query FeedMyChannels($after:ID,$first:Int!){channels:alphaFeedMyChannels(after:$after,first:$first){__typename cursor items{__typename ...FeedChannelFull}}}fragment FeedChannelFull on FeedChannel{__typename about id isGlobal myRole photo shortname socialImage subscribed subscribersCount title}",
        FeedMyChannelsSelector
    )
    let FeedRecommendedChannels = OperationDefinition(
        "FeedRecommendedChannels",
        .query, 
        "query FeedRecommendedChannels($after:String,$first:Int!){search:alphaRecommendedChannels(after:$after,first:$first){__typename edges{__typename cursor node{__typename ...FeedChannelFull}}pageInfo{__typename currentPage hasNextPage hasPreviousPage itemsCount openEnded pagesCount}}}fragment FeedChannelFull on FeedChannel{__typename about id isGlobal myRole photo shortname socialImage subscribed subscribersCount title}",
        FeedRecommendedChannelsSelector
    )
    let FeedWritableChannels = OperationDefinition(
        "FeedWritableChannels",
        .query, 
        "query FeedWritableChannels($after:ID,$first:Int!){channels:alphaWritableChannels(after:$after,first:$first){__typename cursor items{__typename ...FeedChannelFull}}}fragment FeedChannelFull on FeedChannel{__typename about id isGlobal myRole photo shortname socialImage subscribed subscribersCount title}",
