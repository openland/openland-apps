...
class Operations {
    ...
    let ChatMentionSearch = OperationDefinition(
        "ChatMentionSearch",
        .query, 
        "query ChatMentionSearch($after:String,$cid:ID!,$first:Int!,$query:String){mentions:chatMentionSearch(after:$after,cid:$cid,first:$first,query:$query){__typename cursor globalItems{__typename ... on Organization{...OrganizationShort}... on User{...UserForMention}... on SharedRoom{...RoomSharedNano}}localItems{__typename ...UserForMention}}}fragment OrganizationShort on Organization{__typename about isCommunity:alphaIsCommunity id name photo shortname}fragment UserForMention on User{__typename id isBot isYou name photo primaryOrganization{__typename id name}shortname}fragment RoomSharedNano on SharedRoom{__typename id isChannel kind roomPhoto:photo settings{__typename id mute}title}",
