...
class Operations {
    ...
    let RoomMembersForMentionsPaginated = OperationDefinition(
        ...
        .query, 
        "query RoomMembersForMentionsPaginated($after:ID,$first:Int,$roomId:ID!){members:roomMembers(after:$after,first:$first,roomId:$roomId){__typename user{__typename ...UserForMention}}}fragment UserForMention on User{__typename id name photo primaryOrganization{__typename id name}}",
    ...
    let OnlineWatch = OperationDefinition(
        "OnlineWatch",
        .subscription, 
        "subscription OnlineWatch($conversations:[ID!]!){alphaSubscribeChatOnline(conversations:$conversations){__typename timeout type user:user{__typename id lastSeen online}}}",
        OnlineWatchSelector
    )
