...
class Operations {
    ...
    let ChatJoin = OperationDefinition(
        "ChatJoin",
        .query, 
        "query ChatJoin($id:ID!){room(id:$id){__typename ... on SharedRoom{description id isChannel membersCount onlineMembersCount photo previewMembers{__typename id photo}title}}}",
        ChatJoinSelector
    )
    ...
    let ConferenceMedia = OperationDefinition(
        "ConferenceMedia",
        .query, 
        "query ConferenceMedia($id:ID!,$peerId:ID!){conferenceMedia(id:$id,peerId:$peerId){__typename iceServers{__typename credential urls username}id streams{__typename ice id peerId sdp state}}}",
    ...
    let ExplorePeople = OperationDefinition(
        "ExplorePeople",
        .query, 
        "query ExplorePeople($after:String,$page:Int,$query:String,$sort:String){items:userSearch(after:$after,first:25,page:$page,query:$query,sort:$sort){__typename edges{__typename cursor node{__typename isYou ...UserShort}}pageInfo{__typename currentPage hasNextPage hasPreviousPage itemsCount openEnded pagesCount}}}fragment UserShort on User{__typename email firstName id isBot isYou lastName lastSeen name online photo primaryOrganization{__typename ...OrganizationShort}shortname}fragment OrganizationShort on Organization{__typename about isCommunity:alphaIsCommunity id membersCount name photo shortname}",
    ...
    let FeedChannel = OperationDefinition(
        "FeedChannel",
        .query, 
        "query FeedChannel($id:ID!){channel:alphaFeedChannel(id:$id){__typename ...FeedChannelFull}}fragment FeedChannelFull on FeedChannel{__typename about id isGlobal myRole photo postsCount shortname socialImage subscribed subscribersCount title}",
        FeedChannelSelector
    )
    ...
    let FeedChannelSubscribers = OperationDefinition(
        ...
        .query, 
        "query FeedChannelSubscribers($after:String,$channelId:ID!,$first:Int!,$query:String){subscribers:alphaFeedChannelSubscribers(after:$after,channelId:$channelId,first:$first,query:$query){__typename edges{__typename cursor node{__typename role user{__typename ...UserShort}}}pageInfo{__typename currentPage hasNextPage hasPreviousPage itemsCount openEnded pagesCount}}}fragment UserShort on User{__typename email firstName id isBot isYou lastName lastSeen name online photo primaryOrganization{__typename ...OrganizationShort}shortname}fragment OrganizationShort on Organization{__typename about isCommunity:alphaIsCommunity id membersCount name photo shortname}",
        FeedChannelSubscribersSelector
    )
    let FeedChannelWriters = OperationDefinition(
        "FeedChannelWriters",
        .query, 
        "query FeedChannelWriters($after:ID,$first:Int!,$id:ID!){writers:alphaFeedChannelAdmins(after:$after,first:$first,id:$id){__typename cursor items{__typename role user{__typename ...UserShort}}}}fragment UserShort on User{__typename email firstName id isBot isYou lastName lastSeen name online photo primaryOrganization{__typename ...OrganizationShort}shortname}fragment OrganizationShort on Organization{__typename about isCommunity:alphaIsCommunity id membersCount name photo shortname}",
        FeedChannelWritersSelector
    )
    let FeedChannelsSearch = OperationDefinition(
        "FeedChannelsSearch",
        .query, 
        "query FeedChannelsSearch($after:String,$first:Int!,$query:String,$sort:String){search:alphaFeedChannelSearch(after:$after,first:$first,query:$query,sort:$sort){__typename edges{__typename cursor node{__typename ...FeedChannelFull}}pageInfo{__typename currentPage hasNextPage hasPreviousPage itemsCount openEnded pagesCount}}}fragment FeedChannelFull on FeedChannel{__typename about id isGlobal myRole photo postsCount shortname socialImage subscribed subscribersCount title}",
    ...
    let FeedWritableChannels = OperationDefinition(
        ...
        .query, 
        "query FeedWritableChannels($after:ID,$first:Int!){channels:alphaWritableChannels(after:$after,first:$first){__typename cursor items{__typename ...FeedChannelFull}}}fragment FeedChannelFull on FeedChannel{__typename about id isGlobal myRole photo postsCount shortname socialImage subscribed subscribersCount title}",
    ...
    let InitFeed = OperationDefinition(
        "InitFeed",
        .query, 
        "query InitFeed($first:Int!){drafts:alphaFeedMyDraftsChannel{__typename ...FeedChannelFull}feed:alphaHomeFeed(first:$first){__typename cursor items{__typename ...FeedItemFull}}}fragment FeedChannelFull on FeedChannel{__typename about id isGlobal myRole photo postsCount shortname socialImage subscribed subscribersCount title}fragment FeedItemFull on FeedItem{__typename ... on FeedPost{author{__typename ...FeedPostAuthorFragment}canEdit commentsCount date edited fallback id message reactions{__typename reaction user{__typename ...UserShort}}slides{__typename ...SlideFragment}source{__typename ...FeedPostSourceFragment}}}fragment FeedPostAuthorFragment on FeedPostAuthor{__typename ... on User{...UserShort}}fragment UserShort on User{__typename email firstName id isBot isYou lastName lastSeen name online photo primaryOrganization{__typename ...OrganizationShort}shortname}fragment OrganizationShort on Organization{__typename about isCommunity:alphaIsCommunity id membersCount name photo shortname}fragment SlideFragment on Slide{__typename ... on TextSlide{attachments{__typename ... on User{...UserShort}... on SharedRoom{canSendMessage id kind membersCount membership organization{__typename id name photo}roomPhoto:photo title}... on Organization{...OrganizationShort}}cover{__typename metadata{__typename imageFormat imageHeight imageWidth isImage mimeType name size}url}coverAlign id spans{__typename ...SpanFragment}text}}fragment SpanFragment on MessageSpan{__typename length offset ... on MessageSpanUserMention{user{__typename ...UserForMention}}... on MessageSpanMultiUserMention{users{__typename ...UserForMention}}... on MessageSpanOrganizationMention{organization{__typename ...OrganizationShort}}... on MessageSpanRoomMention{room{__typename ...RoomNano}}... on MessageSpanLink{url}... on MessageSpanDate{date}}fragment UserForMention on User{__typename id isBot isYou name photo primaryOrganization{__typename id name}shortname}fragment RoomNano on Room{__typename ... on PrivateRoom{id settings{__typename id mute}user{__typename id name photo}}... on SharedRoom{...RoomSharedNano}}fragment RoomSharedNano on SharedRoom{__typename id isChannel kind membersCount roomPhoto:photo settings{__typename id mute}title}fragment FeedPostSourceFragment on FeedPostSource{__typename ... on FeedChannel{...FeedChannelFull}}",
        InitFeedSelector
    )
    ...
    let MyOrganizations = OperationDefinition(
        "MyOrganizations",
        .query, 
        "query MyOrganizations{myOrganizations{__typename isPrimary:betaIsPrimary ...OrganizationShort}}fragment OrganizationShort on Organization{__typename about isCommunity:alphaIsCommunity id membersCount name photo shortname}",
    ...
    let OrganizationPublicRooms = OperationDefinition(
        ...
        .query, 
        "query OrganizationPublicRooms($after:ID,$first:Int!,$organizationId:ID!){organizationPublicRooms(after:$after,first:$first,id:$organizationId){__typename cursor items{__typename ...SharedRoomView}}}fragment SharedRoomView on SharedRoom{__typename id membersCount photo photo title}",
    ...
    let RoomInviteInfo = OperationDefinition(
        "RoomInviteInfo",
        .query, 
        "query RoomInviteInfo($invite:String!){invite:betaRoomInviteInfo(invite:$invite){__typename id invitedByUser{__typename ...UserShort}room{__typename ... on SharedRoom{description id isChannel isPremium kind matchmaking{__typename enabled}membersCount membership onlineMembersCount organization{__typename ...OrganizationShort}photo premiumPassIsActive premiumSettings{__typename id interval price}premiumSubscription{__typename id state}previewMembers{__typename id photo}socialImage title}}}}fragment UserShort on User{__typename email firstName id isBot isYou lastName lastSeen name online photo primaryOrganization{__typename ...OrganizationShort}shortname}fragment OrganizationShort on Organization{__typename about isCommunity:alphaIsCommunity id membersCount name photo shortname}",
    ...
    let RoomMembers = OperationDefinition(
        "RoomMembers",
        .query, 
        "query RoomMembers($roomId:ID!){members:roomMembers(roomId:$roomId){__typename canKick membership role user{__typename ...UserShort}}}fragment UserShort on User{__typename email firstName id isBot isYou lastName lastSeen name online photo primaryOrganization{__typename ...OrganizationShort}shortname}fragment OrganizationShort on Organization{__typename about isCommunity:alphaIsCommunity id membersCount name photo shortname}",
        RoomMembersSelector
    )
    ...
    let SharedMediaCounters = OperationDefinition(
        "SharedMediaCounters",
        .query, 
        "query SharedMediaCounters($chatId:ID!){counters:chatSharedMediaCounters(chatId:$chatId){__typename documents images links videos}}",
    ...
    let SuperAdmins = OperationDefinition(
        "SuperAdmins",
        .query, 
        "query SuperAdmins{superAdmins{__typename email role user{__typename ...UserShort}}}fragment UserShort on User{__typename email firstName id isBot isYou lastName lastSeen name online photo primaryOrganization{__typename ...OrganizationShort}shortname}fragment OrganizationShort on Organization{__typename about isCommunity:alphaIsCommunity id membersCount name photo shortname}",
        SuperAdminsSelector
    )
    ...
    let MatchmakingProfileFill = OperationDefinition(
        ...
        .mutation, 
        "mutation MatchmakingProfileFill($input:MatchmakingProfileFillInput!,$peerId:ID!){matchmakingProfileFill(input:$input,peerId:$peerId){__typename ...MatchmakingProfileFragment}}fragment MatchmakingProfileFragment on MatchmakingProfile{__typename answers{__typename ... on TextMatchmakingAnswer{answer question{__typename id subtitle title}}... on MultiselectMatchmakingAnswer{question{__typename id subtitle title}tags}}chatCreated user{__typename id isBot isYou name photo primaryOrganization{__typename id name}}}",
    ...
    let MediaAnswer = OperationDefinition(
        "MediaAnswer",
        .mutation, 
        "mutation MediaAnswer($answer:String!,$id:ID!,$peerId:ID!){mediaStreamAnswer(answer:$answer,id:$id,peerId:$peerId){__typename id streams{__typename ice id peerId sdp state}}}",
        MediaAnswerSelector
    )
    let MediaCandidate = OperationDefinition(
        "MediaCandidate",
        .mutation, 
        "mutation MediaCandidate($candidate:String!,$id:ID!,$peerId:ID!){mediaStreamCandidate(candidate:$candidate,id:$id,peerId:$peerId){__typename id streams{__typename ice id peerId sdp state}}}",
        MediaCandidateSelector
    )
    let MediaFailed = OperationDefinition(
        "MediaFailed",
        .mutation, 
        "mutation MediaFailed($id:ID!,$peerId:ID!){mediaStreamFailed(id:$id,peerId:$peerId){__typename id streams{__typename ice id peerId sdp state}}}",
        MediaFailedSelector
    )
    let MediaNegotiationNeeded = OperationDefinition(
        "MediaNegotiationNeeded",
        .mutation, 
        "mutation MediaNegotiationNeeded($id:ID!,$peerId:ID!){mediaStreamNegotiationNeeded(id:$id,peerId:$peerId){__typename id streams{__typename ice id peerId sdp state}}}",
        MediaNegotiationNeededSelector
    )
    let MediaOffer = OperationDefinition(
        "MediaOffer",
        .mutation, 
        "mutation MediaOffer($id:ID!,$offer:String!,$peerId:ID!){mediaStreamOffer(id:$id,offer:$offer,peerId:$peerId){__typename id streams{__typename ice id peerId sdp state}}}",
        MediaOfferSelector
    )
    ...
    let OrganizationAddMember = OperationDefinition(
        ...
        .mutation, 
        "mutation OrganizationAddMember($organizationId:ID!,$userIds:[ID!]){alphaOrganizationMemberAdd(organizationId:$organizationId,userIds:$userIds){__typename role user{__typename ...UserShort}}}fragment UserShort on User{__typename email firstName id isBot isYou lastName lastSeen name online photo primaryOrganization{__typename ...OrganizationShort}shortname}fragment OrganizationShort on Organization{__typename about isCommunity:alphaIsCommunity id membersCount name photo shortname}",
    ...
    let RoomJoin = OperationDefinition(
        "RoomJoin",
        .mutation, 
        "mutation RoomJoin($roomId:ID!){join:betaRoomJoin(roomId:$roomId){__typename ... on PrivateRoom{id}... on SharedRoom{id}}}",
        RoomJoinSelector
    )
    ...
    let RoomsJoin = OperationDefinition(
        "RoomsJoin",
        .mutation, 
        "mutation RoomsJoin($roomsIds:[ID!]!){join:betaRoomsJoin(roomsIds:$roomsIds){__typename ... on PrivateRoom{id}... on SharedRoom{id}}}",
        RoomsJoinSelector
    )
    ...
    let ConferenceMediaWatch = OperationDefinition(
        ...
        .subscription, 
        "subscription ConferenceMediaWatch($id:ID!,$peerId:ID!){media:alphaConferenceMediaWatch(id:$id,peerId:$peerId){__typename id streams{__typename ice id peerId sdp state}}}",
