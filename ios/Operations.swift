...
class Operations {
    ...
    let Feed = OperationDefinition(
        "Feed",
        .query, 
        "query Feed($after:String,$first:Int!){feed:alphaHomeFeed(after:$after,first:$first){__typename cursor items{__typename ...FeedItemFull}}}fragment FeedItemFull on FeedItem{__typename ... on FeedPost{author{__typename ...FeedPostAuthorFragment}canEdit commentsCount date edited fallback id reactions{__typename reaction user{__typename ...UserShort}}slides{__typename ...SlideFragment}}}fragment FeedPostAuthorFragment on FeedPostAuthor{__typename ... on User{...UserShort}... on Organization{...OrganizationShort}}fragment UserShort on User{__typename email firstName id isBot isYou lastName lastSeen name online photo primaryOrganization{__typename ...OrganizationShort}shortname}fragment OrganizationShort on Organization{__typename about isCommunity:alphaIsCommunity id name photo shortname}fragment SlideFragment on Slide{__typename ... on TextSlide{attachments{__typename ... on User{...UserShort}... on SharedRoom{id isChannel kind membersCount membership roomPhoto:photo title}}cover{__typename metadata{__typename imageFormat imageHeight imageWidth isImage mimeType name size}url}coverAlign id spans{__typename ...SpanFragment}text}}fragment SpanFragment on MessageSpan{__typename length offset ... on MessageSpanUserMention{user{__typename ...UserForMention}}... on MessageSpanMultiUserMention{users{__typename ...UserForMention}}... on MessageSpanRoomMention{room{__typename ... on PrivateRoom{id user{__typename id name}}... on SharedRoom{id title}}}... on MessageSpanLink{url}... on MessageSpanDate{date}}fragment UserForMention on User{__typename id isYou name photo primaryOrganization{__typename id name}shortname}",
        FeedSelector
    )
    ...
    let StickerPack = OperationDefinition(
        "StickerPack",
        .query, 
        "query StickerPack($packId:ID!){stickerPack:stickerPack(id:$packId){__typename ... on StickerPack{id stickers{__typename ... on ImageSticker{id image{__typename uuid}}}title}}}",
        StickerPackSelector
    )
    ...
    let StickerPackAddToCollection = OperationDefinition(
        ...
        .mutation, 
        "mutation StickerPackAddToCollection($packId:ID!){stickerPackAddToCollection:stickerPackAddToCollection(id:$packId)}",
