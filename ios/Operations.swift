...
class Operations {
    ...
    let FeedMyChannels = OperationDefinition(
        "FeedMyChannels",
        .query, 
        "query FeedMyChannels($after:ID,$first:Int!){channels:alphaFeedMyChannels(after:$after,first:$first){__typename cursor items{__typename ...FeedChannelFull}}}fragment FeedChannelFull on FeedChannel{__typename about id isGlobal myRole photo postsCount shortname socialImage subscribed subscribersCount title}",
