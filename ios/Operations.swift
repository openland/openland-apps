...
private let MessageReactionsSelector = obj(
            field("__typename", "__typename", notNull(scalar("String"))),
            field("user", "user", notNull(obj(
                    field("__typename", "__typename", notNull(scalar("String"))),
                    field("id", "id", notNull(scalar("ID"))),
...
class Operations {
    ...
    let UserNano = OperationDefinition(
        "UserNano",
        .query, 
        "query UserNano($id:ID!){user(id:$id){__typename ...UserForMention}}fragment UserForMention on User{__typename id name photo shortname isBot primaryOrganization{__typename id name}}",
        UserNanoSelector
    )
