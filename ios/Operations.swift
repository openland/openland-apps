...
class Operations {
    ...
    let ChatJoin = OperationDefinition(
        "ChatJoin",
        .query, 
        "query ChatJoin($id:ID!){room(id:$id){__typename ... on SharedRoom{__typename id title description photo membersCount onlineMembersCount previewMembers{__typename id photo name}isChannel}}}",
        ChatJoinSelector
    )
    ...
    let ResolveShortName = OperationDefinition(
        "ResolveShortName",
        .query, 
        "query ResolveShortName($shortname:String!){item:alphaResolveShortName(shortname:$shortname){__typename ... on User{__typename id isDeleted}... on Organization{__typename id isDeleted}... on FeedChannel{__typename id}}}",
