...
export class UserInfoProvider
  ...
  getChildContext() {
    ...
    let hasAccount =
      this.props.organization !== null && this.props.organization !== undefined;
    // Where to put this?
    if (this.props.user) {
      trackProfile(
        this.props.user.id!!,
...
class UserInfoReceiver extends React.Component<{
  render: React.ComponentType<UserInfoComponentProps>;
}> {
  static contextTypes = {
    user: PropTypes.object,
  ...
  render() {
    const user = this.context.user as Types.UserShort | null;
    const organization = this.context
      .organization as Types.OrganizationShort | null;
    const isLoggedIn = this.context.isLoggedIn as boolean;
    const isActivated = this.context.isActivated as boolean;
    const isProfileCreated = this.context.isProfileCreated as boolean;
    const isAccountExists = this.context.isAccountExists as boolean;
    const isAccountPicked = this.context.isAccountPicked as boolean;
    const isCompleted = this.context.isCompleted as boolean;
    const isBlocked = this.context.isBlocked as boolean;
    const Wrapped = this.props.render;
    return (
      <Wrapped
        user={user}
