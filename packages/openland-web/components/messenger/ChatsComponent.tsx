      ...
      >
        ...
        <Header className="header">
          <Main>
            ...
            {conv.date && (
              <Date className="date">
                <XDate value={conv.date.toString()} format="datetime_short" />
              </Date>
            )}
          </Main>
          <Content>
            <ContentText
              className={'content' + (conv.unread > 0 ? ' with-unread' : '')}
            >
              {conv.typing || (
                <>
                  {!!conv.message &&
              ...
              {select => {
                ...
                return (
                  <ConversationComponent
                    ...
                    conversation={{
                      sender: i.topMessage
                        ? props.user && i.topMessage.sender.id === props.user.id
                          ? 'You'
                          : i.topMessage.sender.name
                        : undefined,
                      key: i.id,
...
> {
  ...
  keydownHandler = (e: any) => {
    ...
    if (!this.props.emptyState && e.code === 'Escape') {
      if (stayChecker || global.getIsEditMessage()) {
        return;
      }
...
export class ChatsComponent extends React.PureComponent<{
  emptyState: boolean;
}> {
  render() {
    if (!canUseDOM) {
