...
interface RoomExploreProps {
    query: string;
    selectedRooms: Map<string, string> | null;
    onPick: (label: string, value: string) => void;
}

export const RoomExploreFragment = React.memo((props: RoomExploreProps) => {
    const client = useClient();
    const data = client.useRoomSearch({ query: props.query });
        ...
        <XScrollView3 flexGrow={1} flexShrink={1} useDefaultScroll={true}>
            <XView>
                {data.items.edges.map(i => {
                    ...
                    const selected = !!(props.selectedRooms && props.selectedRooms.has(i.node.id));
                    return (
                        <UListItem
                            key={node.id}
                            title={node.title}
                            avatar={{ photo: node.photo, title: node.title, id: node.id }}
                            onClick={() => props.onPick(i.node.title, i.node.id)}
                            rightElement={
                                <XView marginRight={8}>
