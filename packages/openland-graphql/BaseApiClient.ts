...
import { GraphqlClient, GraphqlQueryWatch, GraphqlQueryResult, QueryWatchParameters } from './GraphqlClient';
...
export class BaseApiClient {
    ...
    private getQueryWatch<TQuery, TVars>(cache: Map<String, GraphqlQueryWatch<{}>>, query: string, vars?: TVars, opts?: QueryWatchParameters): GraphqlQueryWatch<TQuery> {
        ...
        if (q.has(key)) {
            ...
            let res = this.client.queryWatch<TQuery, TVars>(query, vars, opts);
            q.set(key, res);
            return res;
        }
    }
