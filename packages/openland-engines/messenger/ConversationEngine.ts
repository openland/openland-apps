...
import { RoomMessageFull } from 'openland-api/fragments/MessageFull';
import { UserShort } from 'openland-api/fragments/UserShort';
import gql from 'graphql-tag';
import {
    RoomMessageFull as MessageFullFragment,
    UserShort as UserShortFragment,
...
export interface DataSourceMessageItem {
    ...
    mentions?: RoomMessageFull_mentions[];
    isSending: boolean;
    attachTop: boolean;
...
export function convertMessage(
    ...
    return {
        ...
        isSending: false,
        attachTop: next
            ? next.sender.id === src.sender.id && isSameDate(next.date, src.date)
            : false,
        attachBottom: prev
            ? prev.sender.id === src.sender.id && isSameDate(prev.date, src.date)
            : false,
...
export class ConversationEngine implements MessageSendHandler {
    ...
    start = async () => {
        ...
        let initialChat = await backoff(async () => {
            try {
                return await this.engine.client.query(RoomHistoryQuery, {
    ...
    loadBefore = async (id?: string) => {
        ...
        if (this.loadingHistory === undefined) {
            ...
            let loaded = await backoff(() =>
                this.engine.client.client.query({
            ...
            let history = [...(loaded.data as any).messages].filter(
    ...
    sendMessage = (text: string, mentions: RoomMessageFull_mentions[] | null) => {
    ...
    private updateHandler = async (event: any) => {
        ...
        if (event.__typename === 'ConversationMessageReceived') {
            ...
            if (event.message.message === '/insane') {
                let account: any = this.engine.client.client.readQuery({
                ...
                this.engine.client.client.writeQuery({
                    ...
                    data: {
                        ...
                        permissions: {
                            ...account.permissions,
                            roles: [...account.permissions.roles, 'feature-insane-buttons'],
                        },
                    },
                });
            }
            ...
            (data as any).messages = (data as any).messages.filter(
                (m: any) => m.id !== event.message.id,
            );
            this.engine.client.client.writeQuery({
                query: RoomHistoryQuery.document,
                variables: { roomId: this.conversationId },
            ...
            (data as any).messages = (data as any).messages.map((m: any) =>
    ...
    private appendMessage = (src: ModelMessage) => {
        ...
        if (isServerMessage(src)) {
            ...
            conv.attachTop =
                prev && prev.type === 'message' ? prev.senderId === src.sender.id : false;
        } else {
            ...
            conv = {
                ...
                file: p.uri
                    ...
                    : undefined,
                attachTop:
                    prev && prev.type === 'message' ? prev.senderId === this.engine.user.id : false,
            };
        }
