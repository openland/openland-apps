...
export const FeedManageSlideAttachment = React.memo((props: FeedManageSlideAttachmentProps) => {
    ...
    const content = (
        <>
            {attachment.__typename === 'SharedRoom' && <ZAvatar photo={attachment.roomPhoto} id={attachment.id} title={attachment.title} size="xx-large" />}
            {(attachment.__typename === 'User' || attachment.__typename === 'Organization') && <ZAvatar photo={attachment.photo} id={attachment.id} title={attachment.name} size="xx-large" />}
