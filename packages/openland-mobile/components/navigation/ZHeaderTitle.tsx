...
class ZHeaderIOSTitle extends React.PureComponent<ZHeaderTitleProps, { leftSize?: number, rightSize?: number }> {
            ...
            <View style={{ position: 'absolute', top: 0, left: 0, right: 0, flexDirection: 'row' }} onLayout={this.handleGlobalLayout}>
                {this.state.leftSize !== undefined && this.state.rightSize !== undefined && (
                    <View style={{ position: 'absolute', top: 0, left: 0, right: 0, flexDirection: 'row', flexWrap: 'nowrap' }}>
                        ...
                        {this.state.rightSize - this.state.leftSize > 0 && <View style={{ flexGrow: this.state.rightSize - this.state.leftSize, flexBasis: this.state.rightSize - this.state.leftSize, flexShrink: 100000, backgroundColor: '#0f0' }} />}
                        <Animated.View style={{ flexShrink: 1, flexDirection: 'row', opacity: this.opacity, flexWrap: 'nowrap', transform: [{ translateX: this.translate }] }}>
                            <View style={{ flexDirection: 'column' }}>
                                {!this.props.titleView && this.props.titleText && <Text style={{ textAlign: 'center', backgroundColor: '#afa' }}>{this.props.titleText}</Text>}
                                {!this.props.titleView && this.props.subtitleText && <Text style={{ textAlign: 'center', backgroundColor: '#afa' }}>{this.props.subtitleText}</Text>}
...
export class ZHeaderTitle extends React.PureComponent<ZHeaderTitleProps> {
    ...
    render() {
        if (this.props.appearance === 'android') {
            ...
            return <ZHeaderIOSTitle {...this.props} />;
        }
    }
}