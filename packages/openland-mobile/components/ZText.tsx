        ...
        if (this.props.text) {
            ...
            let parts = preprocessed.map((v, i) => {
                if (v.type === 'new_line') {
                    ...
                    return <Text key={'link-' + i} style={[this.props.style, this.props.linkify && { color: '#0084fe' }]} onPress={this.props.linkify !== false ? resolveInternalLink(v.link!, () => Linking.openURL(v.link!!)) : undefined}>{v.text}</Text>;
                } else {
