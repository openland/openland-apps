            ...
            >
                {communities.edges.map(e => e.node).map(v => (
                    <ZListItem
                        ...
                        subTitle={<>{v.membersCount + (v.membersCount === 1 ? ' member' : ' members')}<Text style={{ opacity: 0.5 }}> ∙</Text> {v.betaPublicRoomsCount + (v.betaPublicRoomsCount === 1 ? ' chat' : ' chats')}</>}
                ...
                <ASSafeAreaContext.Consumer>
                    {sa => (
                        <View width="100%" height="100%" justifyContent="space-between" alignItems="center" paddingTop={sa.top} paddingBottom={sa.bottom + (small ? 0 : 50)}>
                            <Image marginTop={(theme.type === 'Light' ? -30 : 0) - (small ? 25 : 0)} marginBottom={-25} source={theme.type === 'Dark' ? require('assets/img-unsupported_dark.png') : require('assets/img-unsupported.png')} />
