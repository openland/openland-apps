...
const ControlItem = React.memo((props: { theme: ThemeGlobal, text?: string, icon: NodeRequire, iconColor: string, bgColor: string, onPress?: () => void }) => {
    const { theme, text, icon, iconColor, bgColor, onPress } = props;
...
const RoomControls = React.memo((props: { theme: ThemeGlobal }) => {
    const { theme } = props;
    ...
    return (
        <View style={{ marginTop: 20, paddingHorizontal: 38, flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            <ControlItem
                theme={theme}
                icon={require('assets/ic-mute-glyph-36.png')}
                iconColor={theme.foregroundContrast}
                bgColor={theme.tintOrange}
            />
        </View>
    );
});
