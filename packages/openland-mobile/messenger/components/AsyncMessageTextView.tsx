...
export class AsyncMessageTextView extends React.PureComponent<{
    ...
    render() {
        let preprocessed = preprocessText((this.props.message.title ? this.props.message.title + '\n\n' : '') + this.props.message.text!, this.props.message.mentions);
        let big = false;
        ...
        let parts = preprocessed.map((v, i) => {
            if (v.type === 'new_line') {
                ...
                return <ASText key={'mention-' + i} textDecorationLine="underline" onPress={() => this.props.onUserPress(v.link!)}>{v.text}</ASText>;
            } else {
