...
export const AUTH_ENDPOINT = (canUseDOM ? '/authenticate' : (process.env.API_ENDPOINT ? process.env.API_ENDPOINT + '/auth' : 'http://localhost:9000/auth'));
export const API_AUTH_ENDPOINT = (canUseDOM ? '/api/auth' : (process.env.API_ENDPOINT ? process.env.API_ENDPOINT + '/api/auth' : 'http://localhost:9000/api/auth'));