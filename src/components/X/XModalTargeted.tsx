...
export class XModalTargeted extends React.Component<{ title: string, fullScreen?: boolean, closeOnClick?: boolean, width?: number | string }, { isOpen: boolean }> {
    ...
    render() {
        ...
        return (
            <>
                {targetClone}
                <XModal title={this.props.title} isOpen={this.state.isOpen} onClosed={this.handleClose} style={this.props.fullScreen ? 'full-screen' : 'normal'} width={this.props.width} closeOnClick={this.props.closeOnClick}>
                    {contentClone}
                </XModal>
            </>
        );
