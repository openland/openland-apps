...
export const SignOutButton = withUserInfo<{ className: string }>((props) => {
    if (props.user) {
        return <li className={props.className + ' is-join'}><button onClick={e => { props.doLogout(); }}>Sign Out</button></li>;
    } else {
        return null;
    }
});
...
export const SignInButton = withUserInfo<{ className: string }>((props) => {
    if (!props.user) {
        return <li className={props.className}><button onClick={e => { props.doLogin(); }}>Sign In</button></li>;
    } else {
        return null;
    }
});
...
export const SignUpButton = withUserInfo<{ className: string }>((props) => {
    if (!props.user) {
        return <li className={props.className + ' is-join'}><a target="_blank" href="https://goo.gl/forms/YX8LSpH6jWLzbEj02">Join</a></li>;
    } else {
        return null;
    }
});